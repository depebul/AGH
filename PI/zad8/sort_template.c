...
int main(void) {
	Person person_tab[] = {
		...
		{"Ernest", "Eugenie", M,yes, {30, 5, 2023}}
	...
	switch (to_do) {
		case 1:
            ...
            for (int i = 0; i < n; i++) {
                ...
                strcat(output,new_food.name);
                sprintf(bufor,"%.2f",new_food.price);
                strcat(output,bufor);
                sprintf(bufor,"%d",new_food.amount);
                strcat(output,bufor);
                sprintf(bufor,"%d",new_food.valid_date.day);
                strcat(output,bufor);
                sprintf(bufor,"%d",new_food.valid_date.month);
                strcat(output,bufor);
                new_food.valid_date.year-=2000;
                sprintf(bufor,"%d",new_food.valid_date.year);
                strcat(output,bufor);
	        }
            scanf("%s", buff);
            //strcat(output,buff);
